...
func Convert(w io.Writer, vals ...interface{}) error {
	...
	fmt.Fprintf(w, "// Generated by github.com/exponent-io/gots\n")
	for _, t := range types {
		err := writeType(t, w)
		if err != nil {
			return err
...
func writeType(t reflect.Type, w io.Writer) error {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		tag := tag(f)
		if !tag.exclude {
			err := writeField(w, tag.name, typeToString(f.Type), tag.omitempty)
			if err != nil {
				return err
			}
		}
...
func writeField(w io.Writer, name string, typ string, optional bool) error {
	opt := ""
	if optional {
		opt = "?"
	}
